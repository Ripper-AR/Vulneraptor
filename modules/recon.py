...
PORT_LIST_FILE = "all_ports_list.txt"
...
def load_scan_ports(port_file=PORT_LIST_FILE):
    ...
    if not path.exists():
        raise FileNotFoundError(f"Port list file not found: {path}")
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        port_match = re.match(r"^(\d{1,5})\s+", stripped)
        if not port_match:
            continue
    ...
    if not ports:
        raise ValueError(f"No valid ports found in port list file: {path}")
...
def run_recon_scan(target):
    result = {
        ...
        "target": target,
        "status": "success",
        "port_source": str(PORT_LIST_FILE),
        "ports_scanned": [],
        "host": {
            "reachable": False,
            "os_clues": []
